...
ServerRouter.post("/index/:datasetId", async (req: Request<{}, {}, {

}>, res: Response) => {

 // getDataset (spEndpoint, grafo)


 // queryRepository.getQueries() ad ogni query associ un pattern id e una label, label che verrà usata come collectionName nel mappaggio

  // per ogni query, se ci sono dei dati nel grafo
  // crea un mappaggio sullo sparqlEndpoint di configurazione (dataset -> { PatternId, collectionName, indexId })
  // con l'id risultante dall creazione dell'indice

  // carica tutti i risultati della query nell'indice

  // Dataset.getPatterns() ritorna tutti i mappaggi sul grafo di config

  // Pattern.getPatternInstances() ritorna tutte le istanze dall'indice


})


// ServerRouter.post(/queries) // crea nuova query (stringa, label, patternURI)
...
// ServerRouter.post("/index", async (req: Request<{}, {}, {
//     apiKey: string,
